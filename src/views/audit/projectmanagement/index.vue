<template>
  ...
  <div class="projectmanagement">
    ...
    <el-collapse v-model="activeNames" class="sef-collapse-body">
      <el-collapse-item class="sef-collapse" v-for="(item,index) in tableData" :name="index+1" >
        <template slot="title">
          <span class="sef-collapse-title">{{item.projectName}}</span>
          <div class="inline-block sef-collapse-btn">
            <el-button v-if="userInfo.user.id==item.projectLeaderUuid"  @click.stop="editDialog(item)">编辑</el-button>
            <el-button v-if="item.auditConf == 1&&userInfo.user.id==item.projectLeaderUuid"  @click.stop="confirm_problem(item.managementProjectUuid)">确认整改问题清单</el-button>
            <el-button :disabled="item.auditConf!='1'" @click.stop="run_startProject2(item)">启动整改</el-button>
          </div>
        </template>
